    ...
    // if(typeof window.ethereum !== 'undefined') {
        ...
        try {
            ...
            // const networkId = await ethereum.request({
                //     method: "net_version",
                // });
                const response = await myContractSigner.regularMint(mintNumber , {value: ethers.utils.parseEther(total_price)});
                
            if (1 == parseInt(web3reactContext.networkId)){
                                    ...
                                    { parseInt(supply.totalSupply) >= 5555 ? (
                                        ...
                                        <div>
                                            ...
                                            <button className="mintbtn m-2" disabled={claimingNft ? 1 : 0} onClick={mint}>{claimingNft ? "BUSY" : "MINT"} {mintNumber}</button>
                                                          ...
                                                          <button className="mintbtn m-2" onClick={increaseMintNumber}>+</button>
                                                          <button className="mintbtn m-2" onClick={connectMetamask}>connect with metamask</button>
                                        <button className="mintbtn m-2" onClick={connectWalletConnect}>connect with wconnect</button>
                                        <button className="mintbtn m-2" onClick={connectCoinbase}>connect with coinbaise</button>
                                        <button className="mintbtn m-2" onClick={disconnect}>disconnect</button>
                                                  <br />
